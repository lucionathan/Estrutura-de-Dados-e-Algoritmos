...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
    ...
    private void heapify(int position) {
        if (isValidIndex(position)) {
            ...
            if (isValidIndex(left) && comparator.compare(heap[maior], heap[left]) < 0) {
                maior = left(position);
            }
            if (isValidIndex(right) && comparator.compare(heap[maior], heap[right]) < 0) {
    ...
    private boolean isValidIndex(int index) {
        return index >= 0 && index < size() && heap[index] != null;
    }
